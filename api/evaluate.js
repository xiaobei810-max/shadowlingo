...
async function tencentSoeAssess(pcmBase64, refText) {
  ...
  // 前端传来纯 PCM Base64（16kHz 16-bit mono），直接切片发送（不包 WAV 头）
  ...
  // 切片参数：每片 16000 字节 = 0.5s（16kHz×16bit×mono），间隔 200ms → 2.5x < 3x
  ...
  return new Promise((resolve, reject) => {
    ...
    // 超时 = 切片发送时间 + 15s 等待评测结果
    const sendTime = totalChunks * CHUNK_DELAY + 5000;
